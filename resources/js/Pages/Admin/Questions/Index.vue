            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="mb-6 text-right">
                    <Link :href="route('admin.questions.create')">
                        <PrimaryButton>Tambah Soal Baru</PrimaryButton>
                    </Link>
                </div>
                <div class="bg-white border-2 border-slate-800 overflow-hidden shadow-[8px_8px_0_#1e293b] sm:rounded-2xl">
                        ...
                        <table class="w-full">
                            <thead class="border-b-2 border-slate-800">
                                <tr>
                                    ...
                                    <th class="text-left p-3 font-extrabold text-slate-900">Aksara Terkait</th>
                                    <th class="text-left p-3 font-extrabold text-slate-900">Tipe</th>
                            ...
                            <tbody>
                                <tr v-for="question in questions.data" :key="question.id" class="border-b border-slate-200 hover:bg-slate-50">
                                    <td class="p-3 font-medium">{{ question.body }}</td>
                                    <td class="p-3 text-slate-600 font-sunda text-2xl">{{ question.aksara.character }} ({{ question.aksara.latin }})</td>
                                    <td class="p-3 text-sm text-slate-500">{{ question.type }}</td>
                    ...
                    <div v-if="questions.links.length > 3" class="p-4 border-t-2 border-slate-800">
