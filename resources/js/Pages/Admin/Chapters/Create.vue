...
const submit = () => {
    form.post(route('admin.chapters.store'));
};
</script>

<template>
    <Head title="Admin: Tambah Bab" />
    <AdminLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Tambah Bab Baru</h2>
        </template>
                ...
                <div class="bg-white border-2 border-slate-800 overflow-hidden shadow-[8px_8px_0_#1e293b] sm:rounded-2xl p-6 sm:p-8">
                    <form @submit.prevent="submit" class="max-w-2xl space-y-6">
                        ...
                        <div class="flex items-center gap-4">
                            <PrimaryButton :disabled="form.processing">Simpan Bab</PrimaryButton>
                        </div>
                    </form>
                </div>
            </div>
